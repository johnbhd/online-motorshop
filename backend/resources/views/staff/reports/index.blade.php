...
@php
    $summaryCards = [
        ...
        [
            ...
            'support' => 'total_customers_change',
        ...
        [
            ...
            'support' => 'average_order_value_change',
    ...
    $insights = [
        ...
        [
            'key' => 'most_used_fulfillment',
            'title' => 'Most Used Fulfillment',
...
@section('content')
    ...
    >
        ...
        >
            <div>
                ...
                <p class="mt-2 text-sm text-slate-600 sm:text-base">
                    Monitor sales, orders, customers, products, and overall store performance.
                </p>
            </div>
            ...
            <label class="relative block w-full sm:w-48">
                ...
                <i
                    class="fa-regular fa-calendar pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-orange-600"
                    aria-hidden="true"
                ></i>

                <select
                    class="min-h-11 w-full cursor-pointer appearance-none rounded-lg border border-slate-300 bg-white py-2.5 pl-10 pr-9 text-sm font-semibold text-[#0B1930] outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
                    data-reports-period
                >
        ...
        <section aria-label="Report summary">
            ...
            <div
                class="hidden grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5"
                data-reports-summary-content
            >
                    ...
                    <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p
                                    class="mt-2 text-2xl font-bold tracking-tight text-[#0B1930]"
                                    data-report-summary="{{ $card['key'] }}"
                                >
                                    —
                                </p>
                            </div>
                        ...
                        <p class="mt-2 text-xs text-slate-500">
                            <span
                                ...
                                data-report-support="{{ $card['support'] }}"
                            >
                                —
                            </span>

                            <span data-report-support-label="{{ $card['support'] }}">
                                vs previous period
                            </span>
                        </p>

                        <div class="mt-3 h-10">
                            <canvas data-report-sparkline="{{ $card['sparkline'] }}"></canvas>
                        </div>
                    </article>
                @endforeach
            </div>
        ...
        >
            @foreach ($charts as $chart)
                <article
                    ...
                    data-report-chart-card="{{ $chart['key'] }}"
                >
                    <h2 class="text-base font-semibold text-[#0B1930]">
                        {{ $chart['title'] }}
                    </h2>

                    <div
                        class="mt-4 h-64"
                        data-report-chart-loading="{{ $chart['key'] }}"
                    >
                        <x-skeleton
                            type="text"
                            :lines="5"
                    ...
                    <div
                        class="hidden mt-4 h-64"
                        data-report-chart-content="{{ $chart['key'] }}"
                    >
                        <canvas data-report-chart="{{ $chart['key'] }}"></canvas>
                    </div>

                    <p
                        class="hidden mt-4 text-sm text-slate-500"
                        data-report-chart-empty="{{ $chart['key'] }}"
                    >
        ...
        >
            @foreach ($insights as $insight)
                <article
                    class="hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
                    data-report-insight="{{ $insight['key'] }}"
                >
                    <div class="flex items-start gap-3">
                        <span
                            @class([
                                'inline-flex size-10 shrink-0 items-center justify-center rounded-lg',
                                'bg-orange-50 text-orange-600' => $insight['tone'] === 'orange',
                                'bg-blue-50 text-blue-700' => $insight['tone'] === 'blue',
                            ])
                            aria-hidden="true"
                        >
                        ...
                        <div class="min-w-0">
                            ...
                            <h2
                                class="mt-1 truncate font-semibold text-[#0B1930]"
                                data-report-insight-value="{{ $insight['key'] }}"
                            >
                                —
                            </h2>
                            ...
                            <p
                                ...
                                data-report-insight-support="{{ $insight['key'] }}"
                            >
                                —
                            </p>
                        </div>
